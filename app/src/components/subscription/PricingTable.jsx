...
const PricingTable = ({ suscription }) => {
	...
	async function getPopularPlan() {
		...
		try {
			...
			let plans = {}
			const filteredSubscriptions = isRepresentative
				? subscriptions.filter(
						(subscription) => subscription.type === 'CompanySubscription'
					)
				: subscriptions.filter(
						(subscription) => subscription.type === 'CandidateSubscription'
					)
		...
		<div className='p-4 xl:w-1/3 md:w-1/2 w-full'>
			<div className='h-full p-6 rounded-lg border-2 border-gray-300 flex flex-col relative overflow-hidden'>
				...
				<Link
					to={
						!isCurrentPlan
							? suscription.name === 'Custom'
								? '/support'
								: `/payments/${suscription.name}`
							: '#'
					}
					className={`flex items-center mt-auto border-0 py-2 px-4 w-full focus:outline-non rounded ${
						!isCurrentPlan

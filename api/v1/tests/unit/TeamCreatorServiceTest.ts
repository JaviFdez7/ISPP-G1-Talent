...
} from '../../modules/professional-experience/services/ProfessionalExperienceService'
import {
	createTeamCreator,
	deleteTeamCreator,
	getAllTeamCreatorOfRepresentative,
	getTeamCreatorById,
} from '../../modules/team-creator/services/TeamCreatorService'
...
describe('Create correctly a team as a representative', function () {
	...
	it('T1-should create a team and add some new histories to the history of the representative', async function () {
		try {
			...
			assert.strictEqual(
				newHistory.length,
				newTeams[0].profiles[0].recommendedCandidates.length
			)
		} catch (error: any) {
			console.log(error)
		}
	...
	it(
		'T2,T9-After create a team, the representative' +
			' should be able to read the analysis of the candidates',
		async function () {
			try {
				...
				for (
					var i = 0;
					i < (readableTeam as any).profiles[0].recommendedCandidates.length;
					i++
				) {

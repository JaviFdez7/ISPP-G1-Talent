...
function countLanguages(languages: string[]): Record<string, number> {
	...
	for (const language of languages) {
		...
		if (lowerCaseLanguage in languageCount) languageCount[lowerCaseLanguage]++
		else languageCount[lowerCaseLanguage] = 1
	}
...
function countTechnologies(technologies: string[]): Record<string, number> {
	...
	for (const technology of technologies) {
		...
		if (lowerCaseTechnology in technologyCount) technologyCount[lowerCaseTechnology]++
		else technologyCount[lowerCaseTechnology] = 1
	}
...
function countFields(fields: string[]): Record<string, number> {
	...
	for (const field of fields) {
		...
		if (lowerCaseField in fieldCount) fieldCount[lowerCaseField]++
		else fieldCount[lowerCaseField] = 1
	}
	return fieldCount
}
...
function getGlobalTopRepositories(topRepositories: RepositoryInfo[]): RepositoryInfo[] {
	return topRepositories.sort((a, b) => b.forks + b.stars - (a.forks + a.stars)).slice(0, 3)
}
...
async function getTrendData() {
	...
	for (const analysisItem of analysis) {
		candidateLanguages.push(
			...analysisItem.globalTopLanguages.map((lang: any) => lang.language)
		)

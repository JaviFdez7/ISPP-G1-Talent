...
function getTopLanguagesPullRequest(result: any): LanguagePercentage[] {
	...
	const languagesPercentage = Object.entries(languageCounts).map(([language, count]) => ({
		language,
...
function processGitHubUserInfo(
	result: any,
	languagesSorted: LanguagePercentage[]
): AnalysisDocument {
	...
	const topRepositories: RepositoryInfo[] = []
	...
	user.repositories.nodes.forEach((repo: any, index: number) => {
		if (repo.object?.text) {
			...
			;[
				...
				.filter(
					(dep) =>
						relevantTechnologies.includes(dep.split('/')[1] || dep) &&
						!dep.startsWith('@')
				)
		...
		if (repo.object?.text) {
			const text: string = repo.object.text
		...
		repo.issues.edges.forEach((issue: any) => {
			issue.node.assignees.nodes.forEach((assignee: any) => {
				if (assignee.login === user.login) numberOfClosedIssues++
			})
		})
...
export async function GetUserAnaliseInfo(

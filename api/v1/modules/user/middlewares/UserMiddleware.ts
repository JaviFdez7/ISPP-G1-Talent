...
export const checkUpdatePassword: any = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const user = await User.findById(id)
    if (!user) {
      return ApiResponse.sendError(res, [{ title: 'Not Found', detail: 'User not found' }], 404)
    }
    const isOldPasswordCorrect = await comparePasswords(oldPassword, user.password)
    if (!isOldPasswordCorrect) {
      return ApiResponse.sendError(res, [{ title: 'Unauthorized', detail: 'Old password is incorrect' }], 401)
    }
    if (oldPassword === newPassword) {
      return ApiResponse.sendError(res, [{ title: 'Bad Request', detail: 'New password cannot be the same as old password' }], 400)
    }

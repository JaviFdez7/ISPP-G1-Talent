...
export const getNotFavoritesTeamCreatorsFromUser: any = async (userId: any) => {
  try {
    const history = await TeamCreatorHistory.find({}).where('userId').equals(userId).where('favorite').equals(false);
    return history;
  } catch (error) {
    console.error('Error getting history:', error);
    throw error;
...
export const getFavoritesTeamCreatorsFromUser: any = async (userId: any) => {
  try {
    const history = await TeamCreatorHistory.find({}).where('userId').equals(userId).where('favorite').equals(true);
    return history;
  } catch (error) {
    console.error('Error getting history:', error);
    throw error;
...
export const toggleFavoriteTeamCreator: any = async (id: any) => {
  try {
    ...
    const updatedHistory = await TeamCreatorHistory.findByIdAndUpdate(id, { favorite: !(history.favorite) }, { new: true });
    return updatedHistory;
  } catch (error) {
    console.error('Error updating history:', error);
    throw error;

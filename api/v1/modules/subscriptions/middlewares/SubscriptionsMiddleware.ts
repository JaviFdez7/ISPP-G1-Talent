...
export const checkUpdateSubscriptionByUserId: any = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const checkGetSubscriptionByUserId: any = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const id = req.params.userId.toString();
      const token = req.headers.authorization ?? '';
      const user = await User.findById(id);
